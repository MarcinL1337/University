...
def opt_dist(line, setup):
    how_many = len(setup) # Ilość elementów na wejściu w wierszu (ile pudełek w wierszu)
    length = len(line) # Długość wiersza (ile pól)
    ...
    # Liczy liczbę inwersji między potencjalnie dobrymi ułożeniami, a wylosowaną linią
    def find_min(choice):
        temp_sum = 0
        for idx, x in enumerate(choice):
    ...
    #Zamienia potencjalnie dobrego tuple'a z rozmieszczeniem pudełek, na tablicę 0 i 1
    ...
    # wszystkie kombinacje długości how_many, tworzy potencjalne rozmieszczenia pudełek w wierszu
    ...
    # Wybiera wszystkie potencjalne nowe linie, które mają min. liczbę inwersji, a następnie losuje jedną z nich
