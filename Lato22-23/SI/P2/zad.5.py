...
def h(positions, moves):
    distances = [dist[pos] for pos in positions]
    return max(distances) + moves # do 6 pomnożyć przez stałą, np. 2

def Astar(K, M):
    Q = []
    heapq.heappush(Q, (h(K, 0), K, M))
    allpos.add(tuple(K))
    ...
    while len(Q) > 0:
        ...
        for i in range(4):
            ...
            if hasz not in allpos:
                allpos.add(hasz)
                heapq.heappush(Q, (h(temp_pos, len(moves) + 1), temp_pos, moves + L[i]))
