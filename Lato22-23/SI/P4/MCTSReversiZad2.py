...
class GameState:
    def __init__(self, startingPlayer=1, ourPlayer=1):
        self.board = self.initial_board()
        self.toPlay = startingPlayer
        self.freePos = self.freePositions()
        self.whiteLegalMoves = self.getLegalMoves(1)
        self.blackLegalMoves = self.getLegalMoves(0)
    ...
    def result(self):
        ...
        for i in range(8):
            for j in range(8):
                if self.board[i][j] == self.ourPlayer:
                    res += 1
                elif self.board[i][j] == 1 - self.ourPlayer:
                    res -= 1
    ...
    def randomMove(self):
        ...
        if idle in poss_moves:
            ...
            return idle
        # x, y = poss_moves.pop()
        # x, y = random.choice(list(poss_moves))
        x, y = random.sample(poss_moves, 1)[0]
