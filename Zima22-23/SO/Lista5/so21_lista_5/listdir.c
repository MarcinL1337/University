...
static void print_mode(mode_t m) {
  ...
  ux = (S_ISUID & m) ? ((m & S_IXUSR) ? 's' : 'S') : ux;
  gx = (S_ISGID & m) ? ((m & S_IXGRP) ? 's' : 'S') : gx;
  ox = (S_ISVTX & m) ? ((m & S_IXOTH) ? 't' : 'T') : ox;
...
static void file_info(int dirfd, const char *name) {
  ...
  /* TODO: Read where symlink points to and print '-> destination' string. */
    char buff[1024];
    int n = Readlinkat(dirfd, name, buff, sizeof(buff));
    buff[n] = '\0';
    printf("-> %s", buff);
  }
...
int main(int argc, char *argv[]) {
  ...
  while ((n = Getdents(dirfd, (void *)buf, DIRBUFSZ))) {
    ...
    void* end = n+buf;
    void* temp_buf = buf;
    while ((void*)temp_buf < end) {
      d = (struct linux_dirent*)temp_buf;
