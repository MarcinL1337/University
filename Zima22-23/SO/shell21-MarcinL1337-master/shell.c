...
static int do_pipeline(token_t *token, int ntokens, bool bg) {
  ...
  int i = 0;
  token_t *cur_process = (token_t)malloc(ntokens*sizeof(token_t));
  ...
  pgid = do_stage(0, &mask, input, output, cur_process, i-1, bg);
  ...
  while(token[i] != NULL)
  {
    cur_process[j] = token[i];
    j++;
    
    if(token[i] == T_PIPE)
    {
      pid = do_stage(pgid, &mask, input, output, cur_process, j-1, bg);
      setpgid(pid, pgid);
      j = 0;
    }
  }
