...
static void sigchld_handler(int sig) {
  ...
  for(int i = 0; i < njobmax; i++)
  {
    ...
    {
      pid = waitpid(jobs[i].proc->pid, &status, WNOHANG|WUNTRACED|WCONTINUED);
      if(pid > 0)
      {
      ...
      int fin = 0, stp = 0, run = 0, cur_state = 0;
      
      for(int k = 0; k < jobs[i].nproc; k++)
      {
        switch(jobs[i].proc[k].state)
        {
          case FINISHED: fin++; break;
          case STOPPED:  stp++; break;
      ...
      else if(jobs[i].nproc == stp) //if(stp > 0)??
  ...
  for (int j = BG; j < njobmax; j++) {
    ...
    if(which == cur_state || which == ALL)
    {
      if(cur_state == FINISHED)
      {
        ...
        {
          printf("[%d] killed '%s' by signal %d\n", j, jobs[j].command, exitcode(&jobs[j]));
        }
        deljob(&jobs[j]);
      }
      else if(cur_state == RUNNING)
